...
exports.GeneratePasswordLink = async (req, res, next) => {
    try {
        ...
        if (!user[0].mailVerified) {
            throw new AuthenticationError("Email not verified yet. Please verify it...");
        }
        ...
        return res.status(200).json({
            message: "Password link has been sent on your registered email successfully!!",
            success: true
        });
...
exports.SetPassword = async (req, res, next) => {
    try {
        ...
        if (user[0].verifyTokenExpiry < Date.now()) {
            throw new AuthenticationError("invalid/expired url... please generate new link");
        }
        ...
        //for patient
        if (last_sym === "@") {
            await Patient.findByIdAndUpdate(user[0].id, { verifyToken: null, verifyTokenExpiry: null, password: newpassword });
        }
        //for doctor
        else {
